...
func newHandler() courier.ChannelHandler {
	return &handler{handlers.NewBaseHandler(courier.ChannelType(channelTypeTrn), "Turn.io WhatsApp")}
}
...
func (h *handler) receiveEvents(ctx context.Context, channel courier.Channel, w http.ResponseWriter, r *http.Request, payload *eventsPayload, clog *courier.ChannelLog) ([]courier.Event, error) {
	...
	for _, msg := range payload.Messages {
		...
		// create our URN
		sender := whatsapp.Identifier(msg.From, msg.FromBSUID)
...
func (h *handler) fetchMediaID(msg courier.MsgOut, mimeType, mediaURL string, clog *courier.ChannelLog) (string, error) {
	...
	if err != nil {
		return "", errors.Wrapf(err, "invalid base url set for TRN channel: %s", baseURL)
	}
...
func (h *handler) sendWhatsAppMsg(rc redis.Conn, msg courier.MsgOut, sendPath *url.URL, payload interface{}, clog *courier.ChannelLog) (string, string, error) {
