...
func TestVerify(t *testing.T) {
	...
	RunChannelTestCases(t, testChannelsWAC, newHandler("WAC", "WhatsApp Cloud", false), []ChannelHandleTestCase{
		{
			Label:                 "WAC Valid Secret",
			URL:                   "/c/wac/receive?hub.mode=subscribe&hub.verify_token=wac_webhook_secret&hub.challenge=yarchallenge",
			ExpectedRespStatus:    200,
			ExpectedBodyContains:  "yarchallenge",
			NoQueueErrorCheck:     true,
			NoInvalidChannelCheck: true,
		...
		{
			Label:                "WAC Verify No Mode",
			URL:                  "/c/wac/receive",
			ExpectedRespStatus:   400,
			ExpectedBodyContains: "unknown request",
		},
		{
			Label:                "WAC Verify No Secret",
			URL:                  "/c/wac/receive?hub.mode=subscribe",
			ExpectedRespStatus:   400,
			ExpectedBodyContains: "token does not match secret",
		},
		{
			Label:                "WAC Invalid Secret",
			URL:                  "/c/wac/receive?hub.mode=subscribe&hub.verify_token=blah",
			ExpectedRespStatus:   400,
			ExpectedBodyContains: "token does not match secret",
		},
		{
